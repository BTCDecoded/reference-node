...
#[derive(Clone)]
struct BenchClock { now: Instant }
impl BenchClock { fn new() -> Self { Self { now: Instant::now() } } fn advance(&mut self, d: Duration) { self.now += d; } }
impl Clock for BenchClock { fn now(&self) -> Instant { self.now } }
    ...
    group.bench_function("start_and_advance_10k", |b| {
        b.iter_batched(
            || {
                ...
                let mut d: DandelionRelay<BenchClock> = DandelionRelay::with_rng_and_clock(rng, clock);
