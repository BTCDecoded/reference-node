...
impl ManifestValidator {
    ...
    pub fn validate(&self, manifest: &ModuleManifest) -> ValidationResult {
        ...
        if manifest.version.is_empty() {
            ...
            errors.push(format!("Invalid version format: {} (expected semantic versioning)", manifest.version));
        }
        ...
        if !self.is_valid_name(&manifest.name) {
            errors.push(format!("Invalid module name: {} (must be alphanumeric with dashes/underscores)", manifest.name));
        }
    ...
    fn is_valid_version(&self, version: &str) -> bool {
        ...
        nums.iter().all(|n| !n.is_empty() && n.chars().all(|c| c.is_ascii_digit()) && n.parse::<u32>().is_ok())
    }
    ...
    /// Validate dependencies
    fn validate_dependencies(&self, dependencies: &HashMap<String, String>) -> Result<(), Vec<String>> {
        let mut errors = Vec::new();
        ...
        for (dep_name, dep_version) in dependencies {
            ...
            if !self.is_valid_version_or_range(dep_version) {
                errors.push(format!("Invalid dependency version format: {} (for dependency: {})", dep_version, dep_name));
            }
        }
    ...
    fn is_valid_version_or_range(&self, version: &str) -> bool {
        ...
        if version.starts_with(">=") || version.starts_with("<=") || version.starts_with("==") 
            ...
            let version_part = version.trim_start_matches(|c: char| c == '>' || c == '=' || c == '^' || c == '~' || c == '<');
...
pub fn validate_module_signature(_manifest: &ModuleManifest, _binary_path: &std::path::Path) -> Result<(), ModuleError> {
