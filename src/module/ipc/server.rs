...
use crate::module::ipc::protocol::{ModuleMessage, RequestMessage, ResponseMessage, RequestPayload};
...
impl ModuleIpcServer {
    ...
    pub async fn start<A: NodeAPI + Send + Sync + 'static>(&mut self, node_api: Arc<A>) -> Result<(), ModuleError> {
        ...
        if let Some(parent) = self.socket_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| ModuleError::IpcError(format!("Failed to create socket directory: {}", e)))?;
        }
    ...
    async fn handle_connection<A: NodeAPI + Send + Sync>(&mut self, stream: UnixStream, node_api: Arc<A>) -> Result<(), ModuleError> {
        ...
        let writer_task_handle = tokio::spawn(async move {
            ...
            tokio::spawn(async move {
                while let Some(event_message) = event_rx.recv().await {
                    match bincode::serialize(&event_message) {
                        Ok(bytes) => {
                            if outgoing_tx_for_events.send(bytes::Bytes::from(bytes)).is_err() {
                                break; // Receiver dropped, connection closed
                            }
                        }
                        Err(e) => {
                            warn!("Failed to serialize event for module {}: {}", module_id_event_fwd, e);
                        }
                    }
                }
                ...
                if let Some(event_mgr) = event_manager_clone {
                    if let Err(e) = event_mgr.unsubscribe_module(&module_id_event_fwd).await {
                        warn!("Failed to unsubscribe module {} from events: {}", module_id_event_fwd, e);
                    }
                }
            });
            ...
            while let Some(bytes) = outgoing_rx.recv().await {
                if let Err(e) = writer.send(bytes).await {
                    warn!("Failed to send message to module {}: {}", module_id_writer_task, e);
                    break;
                }
            }
        });
        ...
        while let Some(result) = connection.reader.next().await {
            match result {
                Ok(bytes) => {
                    let node_api_clone = Arc::clone(&node_api);
                    match self.handle_message(bytes.as_ref(), &mut connection, node_api_clone).await {
                        Ok(()) => {}
                        Err(e) => {
        ...
        if let Some(mut conn) = self.connections.remove(&module_id) {
            ...
            if let Some(event_mgr) = &self.event_manager {
                if let Err(e) = event_mgr.unsubscribe_module(&module_id).await {
                    warn!("Failed to unsubscribe module {} from events: {}", module_id, e);
                }
            }
        }
        ...
        let message: ModuleMessage = bincode::deserialize(bytes)
            ...
            ModuleMessage::Request(request) => {
                ...
                if let RequestPayload::SubscribeEvents { ref event_types } = request.payload {
                    if let Some(event_mgr) = &self.event_manager {
                        if let Some(event_tx) = &connection.event_tx {
                            ...
                            event_mgr.subscribe_module(module_id.clone(), event_types.clone(), event_tx_clone).await?;
                            connection.subscriptions = event_types.clone();
                            debug!("Module {} subscribed to events: {:?}", module_id, event_types);
                        }
                    }
                }
                ...
                let response = if let Some(hub) = &self.api_hub {
                    ...
                    hub_guard.handle_request(&connection.module_id, request.clone()).await?
                } else {
                ...
                if let Some(tx) = &connection.outgoing_tx {
                    tx.send(bytes::Bytes::from(response_bytes))
                        .map_err(|e| ModuleError::IpcError(format!("Failed to send response: {}", e)))?;
                }
            }
    ...
    async fn process_request<A: NodeAPI + Send + Sync>(
        ...
        match &request.payload {
            RequestPayload::GetBlock { hash } => {
                ...
                Ok(ResponseMessage::success(request.correlation_id, ResponsePayload::Block(block)))
            }
            RequestPayload::GetBlockHeader { hash } => {
                let header = node_api.get_block_header(hash).await?;
                Ok(ResponseMessage::success(request.correlation_id, ResponsePayload::BlockHeader(header)))
            }
            RequestPayload::GetTransaction { hash } => {
                let tx = node_api.get_transaction(hash).await?;
                Ok(ResponseMessage::success(request.correlation_id, ResponsePayload::Transaction(tx)))
            }
            RequestPayload::HasTransaction { hash } => {
                let exists = node_api.has_transaction(hash).await?;
                Ok(ResponseMessage::success(request.correlation_id, ResponsePayload::Bool(exists)))
            }
            RequestPayload::GetChainTip => {
                let tip = node_api.get_chain_tip().await?;
                Ok(ResponseMessage::success(request.correlation_id, ResponsePayload::Hash(tip)))
            }
            RequestPayload::GetBlockHeight => {
                let height = node_api.get_block_height().await?;
                Ok(ResponseMessage::success(request.correlation_id, ResponsePayload::U64(height)))
            }
            RequestPayload::GetUtxo { outpoint } => {
                let utxo = node_api.get_utxo(outpoint).await?;
                Ok(ResponseMessage::success(request.correlation_id, ResponsePayload::Utxo(utxo)))
            }
            RequestPayload::SubscribeEvents { event_types } => {
                ...
                Ok(ResponseMessage::success(request.correlation_id, ResponsePayload::SubscribeAck))
            }
        }
    }
}
