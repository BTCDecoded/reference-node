...
impl NetworkSandbox {
    ...
    pub fn validate_network_operation(&self, operation: &str) -> Result<(), ModuleError> {
        if !self.allow_network {
            warn!("Module attempted network operation: {} (network access denied)", operation);
    ...
    pub fn validate_endpoint(&self, endpoint: &str) -> Result<(), ModuleError> {
        ...
        if !self.allowed_endpoints.is_empty() {
            let allowed = self.allowed_endpoints.iter()
                .any(|e| endpoint.starts_with(e));
            
            if !allowed {
                warn!("Module attempted to connect to unauthorized endpoint: {}", endpoint);
                return Err(ModuleError::OperationError(format!(
