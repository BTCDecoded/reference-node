...
use crate::module::traits::{ModuleError, EventType};
use crate::module::ipc::protocol::{ModuleMessage, EventMessage, EventPayload};
...
impl EventManager {
    ...
    pub async fn subscribe_module(
        ...
        info!("Module {} subscribing to events: {:?}", module_id, event_types);
    ...
    pub async fn publish_event(&self, event_type: EventType, payload: EventPayload) -> Result<(), ModuleError> {
        ...
        let channels_snapshot: Vec<(String, mpsc::Sender<ModuleMessage>)> = {
            ...
            module_ids.iter()
                .filter_map(|id| {
                    channels.get(id).map(|sender| (id.clone(), sender.clone()))
                })
                .collect()
        };
