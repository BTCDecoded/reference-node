...
use crate::module::traits::{NodeAPI, ModuleError, EventType};
...
impl NodeAPI for NodeApiImpl {
    ...
    async fn get_block_header(&self, hash: &Hash) -> Result<Option<BlockHeader>, ModuleError> {
        ...
        tokio::task::spawn_blocking({
            ...
            let hash = *hash;
            move || {
                storage.blocks().get_header(&hash)
                    .map_err(|e| ModuleError::OperationError(format!(
    ...
    async fn get_transaction(&self, hash: &Hash) -> Result<Option<Transaction>, ModuleError> {
        ...
        tokio::task::spawn_blocking({
            ...
            let hash = *hash;
            move || {
                storage.transactions().get_transaction(&hash)
                    .map_err(|e| ModuleError::OperationError(format!(
    ...
    async fn has_transaction(&self, hash: &Hash) -> Result<bool, ModuleError> {
        ...
        tokio::task::spawn_blocking({
            ...
            let hash = *hash;
            move || {
                storage.transactions().has_transaction(&hash)
                    .map_err(|e| ModuleError::OperationError(format!(
        ...
        tokio::task::spawn_blocking({
            ...
            move || {
                storage.chain().get_height()
                    ...
                    .ok_or_else(|| ModuleError::OperationError(
                        "Chain not yet initialized".to_string()
                    ))
            }
        }).await
            .map_err(|e| ModuleError::OperationError(format!("Task join error: {}", e)))?
    ...
    async fn get_utxo(&self, outpoint: &OutPoint) -> Result<Option<UTXO>, ModuleError> {
        ...
        tokio::task::spawn_blocking({
            let storage = Arc::clone(&self.storage);
            move || {
                storage.utxos().get_utxo(&outpoint_clone)
                    .map_err(|e| ModuleError::OperationError(format!(
                        "Failed to get UTXO: {}", e
                    )))
            }
        }).await
            .map_err(|e| ModuleError::OperationError(format!("Task join error: {}", e)))?
