...
use crate::module::traits::{ModuleError, ModuleState, ModuleContext, ModuleMetadata};
use crate::module::process::{spawner::{ModuleProcessSpawner, ModuleProcess}, monitor::ModuleProcessMonitor};
use crate::module::ipc::server::ModuleIpcServer;
use crate::module::registry::{ModuleDiscovery, DiscoveredModule, ModuleDependencies};
...
impl ModuleManager {
    ...
    pub async fn start<P: AsRef<Path>, A: crate::module::traits::NodeAPI + Send + Sync + 'static>(
        &mut self,
        socket_path: P,
        node_api: Arc<A>,
    ) -> Result<(), ModuleError> {
        ...
        // Create API hub
        let api_hub = Arc::new(tokio::sync::Mutex::new(ModuleApiHub::new(Arc::clone(&node_api))));
        ...
        let server_handle = tokio::spawn(async move {
            ipc_server.start(node_api_clone).await
        });
    ...
    pub async fn load_module(
        ...
        if modules.contains_key(module_name) {
            return Err(ModuleError::OperationError(format!(
                "Module {} is already loaded", module_name
            )));
        }
        
        // Create module context
        let module_id = format!("{}_{}", module_name, uuid::Uuid::new_v4().to_string());
        let socket_path = self.spawner.socket_dir.join(format!("{}.sock", module_name));
        ...
        let process = self.spawner.spawn(module_name, binary_path, context).await?;
        ...
        let monitor_handle = tokio::spawn(async move {
            if let Err(e) = monitor.monitor_module(module_name_clone.clone(), process).await {
    ...
    pub async fn reload_module(
        ...
        // Load again
        self.load_module(module_name, binary_path, metadata, config).await
    }
