...
#[cfg(kani)]
mod kani_proofs {
    use crate::storage::chainstate::{ChainState, ChainInfo, ChainParams};
    ...
    fn verify_height_consistency() {
        ...
        chain_state.update_tip(&new_hash, &new_header, new_height).unwrap();
    ...
    fn verify_tip_hash_consistency() {
        ...
        chain_state.update_tip(&new_hash, &new_header, height).unwrap();
    ...
    fn verify_chain_work_monotonicity() {
        ...
        let genesis_hash = chain_state.get_tip_hash().unwrap().unwrap();
        let initial_chainwork = chain_state.get_chainwork(&genesis_hash).unwrap().unwrap_or(0);
        ...
        assert!(chainwork1 >= initial_chainwork, "Chainwork must be monotonic (non-decreasing)");
        ...
        // Chainwork should be non-decreasing
        assert!(chainwork2 >= chainwork1, "Chainwork must be monotonic (non-decreasing)");
    }
    ...
    fn verify_chain_work_calculation() {
        ...
        let genesis_hash = chain_state.get_tip_hash().unwrap().unwrap();
        let genesis_chainwork = chain_state.get_chainwork(&genesis_hash).unwrap().unwrap_or(0);
        ...
        if header1.bits > 0 {
            assert!(work1 > 0 || genesis_work > 0, "At least one block should have work");
        }
    }
}
