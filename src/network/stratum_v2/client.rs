...
use crate::network::transport::{Transport, TransportConnection, TransportAddr, TransportType};
...
use crate::network::stratum_v2::protocol::{TlvEncoder, TlvDecoder};
...
pub struct StratumV2Client {
    ...
    pending_requests: Arc<RwLock<std::collections::HashMap<u32, tokio::sync::oneshot::Sender<Vec<u8>>>>>,
...
impl StratumV2Client {
    ...
    pub async fn connect(&mut self) -> StratumV2Result<()> {
        info!("Connecting to Stratum V2 pool: {} (transport: {:?})", self.pool_url, self.transport_type);
        ...
        let connection: Box<dyn TransportConnection + Send> = {
            match self.transport_type {
                TransportType::Tcp => {
                    ...
                    let conn = tcp_transport.connect(transport_addr.clone()).await
                        .map_err(|e| StratumV2Error::Connection(anyhow::anyhow!("TCP connection failed: {}", e)))?;
                    Box::new(conn)
                }
                #[cfg(feature = "quinn")]
                TransportType::Quinn => {
                    let quinn_transport = QuinnTransport::new()
                        .map_err(|e| StratumV2Error::Connection(anyhow::anyhow!("Quinn transport creation failed: {}", e)))?;
                    let conn = quinn_transport.connect(transport_addr.clone()).await
                        .map_err(|e| StratumV2Error::Connection(anyhow::anyhow!("Quinn connection failed: {}", e)))?;
                    Box::new(conn)
                }
                #[cfg(feature = "iroh")]
                TransportType::Iroh => {
                    let iroh_transport = IrohTransport::new().await
                        .map_err(|e| StratumV2Error::Connection(anyhow::anyhow!("Iroh transport creation failed: {}", e)))?;
                    let conn = iroh_transport.connect(transport_addr).await
                        .map_err(|e| StratumV2Error::Connection(anyhow::anyhow!("Iroh connection failed: {}", e)))?;
                    Box::new(conn)
                }
                #[cfg(not(any(feature = "quinn", feature = "iroh")))]
                _ => {
                    return Err(StratumV2Error::Configuration("Transport feature not enabled".to_string()));
                }
            }
        };
        ...
        match tag {
            message_types::SETUP_CONNECTION_SUCCESS => {
                let success_msg: SetupConnectionSuccessMessage = SetupConnectionSuccessMessage::from_bytes(&payload)?;
                info!("Setup Connection successful: versions {:?}, capabilities {:?}", 
            ...
            message_types::SETUP_CONNECTION_ERROR => {
                let error_msg: SetupConnectionErrorMessage = SetupConnectionErrorMessage::from_bytes(&payload)?;
                Err(StratumV2Error::Connection(anyhow::anyhow!("Setup Connection failed: {} (code: {})", 
                    error_msg.error_message, error_msg.error_code)))
            }
            _ => {
                Err(StratumV2Error::InvalidMessageType(tag))
            }
        }
    }
    ...
    pub async fn get_current_job(&self) -> StratumV2Result<Option<NewMiningJobMessage>> {
        if let Some(ref miner) = self.miner {
            ...
            Err(StratumV2Error::MiningJob("Not connected to pool".to_string()))
        }
    }
    ...
    pub async fn submit_shares(&self, shares: Vec<ShareData>) -> StratumV2Result<()> {
        ...
        if !is_connected {
            return Err(StratumV2Error::Connection(anyhow::anyhow!("Not connected to pool")));
        }
            ...
            if let Some(channel_id) = miner.channel_id() {
                ...
                match tag {
                    message_types::SUBMIT_SHARES_SUCCESS => {
                        let success_msg: SubmitSharesSuccessMessage = SubmitSharesSuccessMessage::from_bytes(&payload)?;
                        info!("Shares submitted successfully: channel_id={}, last_job_id={}", 
                              success_msg.channel_id, success_msg.last_job_id);
                        Ok(())
                    }
                    message_types::SUBMIT_SHARES_ERROR => {
                        let error_msg: SubmitSharesErrorMessage = SubmitSharesErrorMessage::from_bytes(&payload)?;
                        warn!("Share submission failed: {} (code: {})", error_msg.error_message, error_msg.error_code);
                ...
                Err(StratumV2Error::MiningJob("No open mining channel".to_string()))
            }
        } else {
            Err(StratumV2Error::Connection(anyhow::anyhow!("Miner not initialized")))
        }
    }
    ...
    fn parse_pool_url(&self, url: &str) -> StratumV2Result<TransportAddr> {
        if url.starts_with("tcp://") {
            let addr_str = url.strip_prefix("tcp://")
                .ok_or_else(|| StratumV2Error::Configuration("Invalid TCP URL format".to_string()))?;
            let socket_addr = addr_str.parse::<SocketAddr>()
                .map_err(|e| StratumV2Error::Configuration(format!("Invalid TCP address: {}", e)))?;
            ...
            #[cfg(feature = "quinn")]
            {
                ...
                let addr_str = url.strip_prefix("quinn://")
                    .ok_or_else(|| StratumV2Error::Configuration("Invalid Quinn URL format".to_string()))?;
                let socket_addr: SocketAddr = addr_str.parse()
                    .map_err(|e| StratumV2Error::Configuration(format!("Invalid Quinn address: {}", e)))?;
                Ok(TransportAddr::Quinn(socket_addr))
            }
            #[cfg(not(feature = "quinn"))]
            {
                Err(StratumV2Error::Configuration("Quinn feature not enabled".to_string()))
            }
            ...
            #[cfg(feature = "iroh")]
            {
                ...
                let node_id_hex = url.strip_prefix("iroh://")
                    .ok_or_else(|| StratumV2Error::Configuration("Invalid Iroh URL format".to_string()))?;
                // Use hex crate (already in dependencies via bitcoin_hashes)
                use hex;
                let node_id_bytes = hex::decode(node_id_hex)
                    .map_err(|e| StratumV2Error::Configuration(format!("Invalid Iroh node ID hex: {}", e)))?;
                Ok(TransportAddr::Iroh(node_id_bytes))
            }
            #[cfg(not(feature = "iroh"))]
            {
                Err(StratumV2Error::Configuration("Iroh feature not enabled".to_string()))
            }
        } else {
            // Try parsing as direct SocketAddr (default to TCP)
            let socket_addr = url.parse::<SocketAddr>()
                .map_err(|e| StratumV2Error::Configuration(format!("Invalid address format: {}", e)))?;
            Ok(TransportAddr::Tcp(socket_addr))
        }
    }
}
