...
impl TlvDecoder {
    ...
    pub fn decode(&mut self) -> StratumV2Result<(u16, Vec<u8>)> {
        ...
        self.cursor.read_exact(&mut length_bytes)
            .map_err(|e| StratumV2Error::TlvEncoding(format!("Failed to read length prefix: {}", e)))?;
        let _total_length = u32::from_le_bytes(length_bytes);
        
        // Read tag (2 bytes, little-endian)
        let mut tag_bytes = [0u8; 2];
        self.cursor.read_exact(&mut tag_bytes)
        ...
        let mut length_bytes = [0u8; 4];
        self.cursor.read_exact(&mut length_bytes)
            .map_err(|e| StratumV2Error::TlvEncoding(format!("Failed to read payload length: {}", e)))?;
    ...
    pub fn decode_raw(data: &[u8]) -> StratumV2Result<(u16, Vec<u8>)> {
        if data.len() < 6 {
            return Err(StratumV2Error::TlvEncoding("Insufficient data for TLV header".to_string()));
        }
