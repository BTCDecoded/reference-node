...
impl Peer {
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        let _ = self.message_tx.send(NetworkMessage::PeerConnected(self.addr));
        
        // Handle peer communication
        self.handle_peer_communication().await?;
        
        // Send disconnection notification
        let _ = self.message_tx.send(NetworkMessage::PeerDisconnected(self.addr));
    ...
    async fn process_message(&self, data: &[u8]) -> Result<()> {
        ...
        match command.as_ref() {
            "block" => {
                let _ = self.message_tx.send(NetworkMessage::BlockReceived(data.to_vec()));
            }
            "tx" => {
                let _ = self.message_tx.send(NetworkMessage::TransactionReceived(data.to_vec()));
            }
            "inv" => {
                let _ = self.message_tx.send(NetworkMessage::InventoryReceived(data.to_vec()));
            }
            _ => {
