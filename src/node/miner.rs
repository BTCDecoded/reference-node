...
impl MiningCoordinator {
    ...
    async fn generate_block_template(&mut self) -> Result<Block> {
        ...
        let transactions = self.transaction_selector.select_transactions(&self.mempool_provider);
...
impl MempoolProvider for MockMempoolProvider {
    ...
    fn get_prioritized_transactions(&self, limit: usize) -> Vec<Transaction> {
        self.prioritized_transactions.iter()
            .take(limit)
            .map(|(tx, _)| tx.clone())
            .collect()
    }
...
mod tests {
    ...
    fn test_mining_engine_template_management() {
        ...
        assert!(engine.get_block_template().is_some());
        assert_eq!(engine.get_block_template().unwrap().header.version, template.header.version);
    ...
    fn test_mining_coordinator_creation() {
        ...
        assert_eq!(coordinator.transaction_selector().max_block_size(), 1_000_000);
    }

    #[test]
    fn test_mining_coordinator_with_params() {
        let mempool = MockMempoolProvider::new();
        let coordinator = MiningCoordinator::with_params(
            mempool,
            4,
            2_000_000,
            8_000_000,
        ...
        assert_eq!(coordinator.transaction_selector().max_block_size(), 2_000_000);
        assert_eq!(coordinator.transaction_selector().max_block_weight(), 8_000_000);
        ...
        Transaction {
            ...
            inputs: vec![
                TransactionInput {
                    ...
                    script_sig: vec![0x76, 0xa9, 0x14, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x88, 0xac],
                    sequence: 0xffffffff,
                }
            ],
            outputs: vec![
