...
async fn test_node_creation_with_different_protocols() {
    ...
    assert_eq!(mainnet_node.protocol().get_protocol_version(), protocol_engine::ProtocolVersion::BitcoinV1);
    ...
    assert_eq!(testnet_node.protocol().get_protocol_version(), protocol_engine::ProtocolVersion::Testnet3);
    ...
    assert_eq!(regtest_node.protocol().get_protocol_version(), protocol_engine::ProtocolVersion::Regtest);
}
...
async fn test_mempool_manager_operations() {
    ...
    let tx1 = helpers::valid_transaction();
    // Create a completely different transaction
    let tx2 = Transaction {
        version: 2, // Different version
        inputs: vec![TransactionInput {
            prevout: OutPoint { hash: helpers::random_hash(), index: 1 },
...
async fn test_mempool_manager_eviction() {
    ...
    for i in 0..100 {
        let tx = TestTransactionBuilder::new()
            .add_input(
                OutPoint { hash: helpers::random_hash(), index: 0 },
                vec![0x41, 0x04],
                0xffffffff,
            )
...
async fn test_mempool_manager_conflict_detection() {
    ...
    let tx1 = TestTransactionBuilder::new()
        .add_input(outpoint.clone(), vec![0x41, 0x04], 0xffffffff)
        .add_output(1000, helpers::p2pkh_script(helpers::random_hash20()))
        .build();
    
    let tx2 = TestTransactionBuilder::new()
        .add_input(outpoint, vec![0x41, 0x04], 0xffffffff)
        .add_output(2000, helpers::p2pkh_script(helpers::random_hash20()))
        .build();
...
async fn test_mempool_cleanup_workflow() {
    ...
    let tx1 = helpers::valid_transaction();
    let tx2 = helpers::valid_transaction();
