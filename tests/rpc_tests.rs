...
async fn test_rpc_request_parsing() {
    ...
    assert_eq!(request_value.get("jsonrpc").unwrap().as_str().unwrap(), "2.0");
    assert_eq!(request_value.get("method").unwrap().as_str().unwrap(), "getblockchaininfo");
...
async fn test_rpc_error_response_formatting() {
    ...
    assert_eq!(error_response.get("jsonrpc").unwrap().as_str().unwrap(), "2.0");
    assert!(error_response.get("error").unwrap().is_object());
    assert_eq!(error_response.get("error").unwrap().get("code").unwrap().as_i64().unwrap(), -32601);
}
