...
async fn test_ban_list_cleanup() {
    ...
    assert!(!banned.iter().any(|(addr, _)| *addr == test_addr), 
...
async fn test_ban_list_permanent() {
    ...
    assert!(manager.is_banned(test_addr), "Permanent ban should be active");
    
    // Clean up
    manager.unban_peer(test_addr);
    assert!(!manager.is_banned(test_addr), "Unbanned peer should not be banned");
}
...
async fn test_ban_list_temporary() {
    ...
    assert!(manager.is_banned(test_addr), "Active temporary ban should be active");
